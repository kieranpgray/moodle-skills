...
compatibility:
  tools:
    - name: web_fetch
      note: >
        Uses web_fetch to retrieve Moodle user documentation from docs.moodle.org.
        No MCP server dependency — uses public documentation URLs.
---
...
2. Fetch the ticket using `web_fetch` from `https://moodle.atlassian.net/rest/api/2/issue/MDL-XXXXX` as a fallback.
...
**Notes:**
- `proceed_to_step_5` is always `true` — Step 3 never blocks the pipeline. Even `WORKING_AS_INTENDED` proceeds to Step 5 for final outcome determination.
- `wontfix_pattern_matched: true` triggers the orchestrator's automatic stop checkpoint — the orchestrator will pause and ask the human to confirm before applying `triage/wontfix`.
